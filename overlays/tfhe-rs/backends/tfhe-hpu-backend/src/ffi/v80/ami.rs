...
impl AmiDriver {
    pub fn new(
        ...
        eprintln!("[AMI DEBUG] ami_id={ami_id}, expected amc_ver={amc_ver:?}, display={amc_ver}");
    ...
    pub fn check_version(amc_ver: &Version) -> Result<(), Box<dyn Error>> {
        ...
        let his_path = his_version_file.expand();
        eprintln!(
            "[AMI DEBUG] his_path={}, raw_his_version={:?}, expected={:?}, parsed_major={}, parsed_minor={}",
            his_path,
            his_version,
            amc_ver,
            amc_major,
